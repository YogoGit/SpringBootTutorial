...
I recommend installing Java 25, as it's the most compatible LTS (Long Term Support)
...
following.
    ...
    2026-12-22T13:26:17.102-07:00  INFO 9483 --- [           main] edu.carroll.cs341.Cs341DemoApplication       : Started Cs341DemoApplication in 0.998 seconds (process running for 1.219)
...
application context is created properly for unit tests (src/test/java/edu/carroll/cs341/Cs341DemoApplicationTests.java).
...
file.  In the dependencies sections, add the following lines in the appropriate
location:
...
Note, there are additional configuration steps you may need to take for this
to work properly. Here is an older article that may provide some information
to allow things to work better.

<https://www.youtube.com/watch?v=BvIM9gNTDM4&t=9s>
...
let's create some subpackages to break out the application into
...
Next, let's add validation to the submission method for the login page in
the controller. This is as simple as adding a **@Valid** annotation to the form
you want validated, and adding a BindingResult parameter to the method. With
both of those changes to the method signature, you can simply check for any
errors in the form, and if they exist, return the failed form with the user
input passed back (*).
...
* - Not in all cases, as sending back password fields is now allowed, so any password data sent in will NOT be passed back to the client browser.

** I also removed the println statement, which is considered very bad form.

For the errors to be displayed, we need to make some slight changes to
the HTML template code, to show the error messages (see the span).
...
Next, LoginController gets two more modifications, one to the loginPost
...
The latter accepts a new String parameter named username (note, we didn't
HAVE to use **@RequestParam** since the field is a String and the name of
...
custom error based on server-side persistence (not-yet-done). Note, what was
done here is NOT the way that should be done for username/password validation,
but was done for demonstration/tutorial purposes.  As mentioned in the
comments, if you do this in a real application, I will hunt you down and
...
allowed, which is compiled into the application code.

Save, test, and then commit your code to your repository, and then you
can compare your version to the reference repository. Make sure to test
both valid and invalid logins (using the hard-coded credentials we placed
in LoginController).
...
data that can be fetched even if the application is restarted. The
most common storage mechanism used is a RDBMS (Relational Database
...
done by adding new lines to the file src/main/resources/application.properties:
...
Create a new subpackage named **jpa** and another subpackage under it named
...
So, let's create another package, this one in a new subpackage named
...
So, create an **interface**, which we'll call LoginService, which will
contain all the methods we need to create in order to properly implement a
login service.
...
service. We can cleanup/remove the hard-coded username and mess we made in the
...
As you may be able to infer, the service code knows how to access the database,
and provides a better method interface so that developers can use more
descriptive method names for defining the operations they want to accomplish.
...
the hashCode of our password.  Using the following throwaway class:
...
So, I'd like to store that hash value in my database. Note, there are
...
process that loads the data externally, but for purposes of the tutorial,
we're populating the data using code. (And, the code itself containing the
data should normally never be committed to a git repo, as credentials
...
verified the code works as before, commit it to your repo, and then compare
...
usually running in an environment where the developer will not have direct
...
setting up logging for the applications. This framework is provided already
for all Spring applications, so there is no need to add any additional
dependencies to the build.gradle file.

First, for every class that we want to have logging enabled, we must
generate a shared logger instance. Doing this is relatively
...
logback, using the filename they recommend using, with a few tweaks to
...
With this file in place, for EVERY class that you want to add logging to, you
...
any information in your logs does not contain information that could
compromise the users who are using your system. (For example, never
...
Because the business logic we created is probably the most useful location
...
Create the static Logger for the class. Note, the class name in
LoggerFactor.getLogger() **MUST** match the classname for logging to work
properly. If you screw this up, the program will compile and run, but it
will be difficult to debug issues as the log data will be confusing.
...
as they allow you to do a deeper dive into the behavior of the code if you
...
In /src/main/java/edu/carroll/cs341/jpa/model/Login.java, add the
...
tested, with **Test** appended to the name by convention.
 ...
 src/test/java/edu/carroll/cs341/service/LoginServiceTest.java:
